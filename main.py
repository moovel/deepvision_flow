...
from models.util import flow2rgb, AverageMeter, save_checkpoint
import numpy as np
...
parser = argparse.ArgumentParser(description='PyTorch FlowNet/Deformable FlowNet Training',
...
def main():
    ...
    time_model = timestamp+'_'+args.arch
    save_path = os.path.join(time_model,save_path)
    ...
    if args.sparse or 'KITTI' in args.dataset:
    ...
    model = models.__dict__[args.arch](None).cuda()
    ...
    optimizer = torch.optim.Adam(param_groups, 0.0001, betas=(0.9, 0.99))

    if args.evaluate:
        best_EPE = validate(val_loader, model, 0, output_writers)
        return

    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, [100,150,200], gamma=0.5)
...
def train(train_loader, model, optimizer, epoch, train_writer):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        flow2_EPE = 20 * realEPE(output[0], target, sparse=args.sparse)
        # record loss and EPE
...
def validate(val_loader, model, epoch, output_writers):
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        # compute output
        output = model(input)
        flow2_EPE = 20*realEPE(output, target, sparse=args.sparse)
        # record EPE
        flow2_EPEs.update(flow2_EPE.item(), target.size(0))
        ...
        if i < len(output_writers):  # log first output of first batches
            if epoch == 0:
                ...
                output_writers[i].add_image('GroundTruth', flow2rgb(20 * target[0], max_value=10), 0)
                output_writers[i].add_image('Inputs', (input[0,:3].cpu() + mean_values).clamp(0,1), 0)
                output_writers[i].add_image('Inputs', (input[0,3:].cpu() + mean_values).clamp(0,1), 1)
            output_writers[i].add_image('FlowNet Outputs', flow2rgb(20 * output[0], max_value=10), epoch)
