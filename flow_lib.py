...
def write_flow_clement(flow, filename, save_path):
    ...
    #normalized_flow_map = flow / (np.abs(flow).max())
    
    #upscale value
    
    #normalized_flow_map = normalized_flow_map*255
    
    #u = normalized_flow_map [: , : , 0]
    #v = normalized_flow_map [: , : , 1] 
...
def show_flow(filename, name_to_save):
    """
    ...
    import scipy.misc
    scipy.misc.imsave(name_to_save, img)
